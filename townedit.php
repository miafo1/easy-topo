        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h4>EDIT TOWN
                        <a href="townview.php" class="btn btn-danger float-end">back</a>
                    </h4>
                </div>
                <div class="card-body">
                     <?php
                    if(isset($_GET['id'])) {
                        $town_id = $_GET['id'];
                        $town = "SELECT * FROM town WHERE idtown='$town_id' LIMIT 1";
                        $town_run = mysqli_query($con, $town);

                        if(mysqli_num_rows($town_run) > 0) {
                            $row = mysqli_fetch_array($town_run);
                          ?> 
                <form action="code.php" method="POST">
                     <div class="col-md-6 mb-3">
                                    <label for="">ID</label>
                                    <input type="hidden" required name="id"  value="<?=$row['idtown'];?>" required="" class="form-control">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="">Town Name</label>
                                    <input type="text" required name="nametown"  value="<?=$row['nametown'];?>" required="" class="form-control">
                                </div>

                                 <div class="col-md-6 mb-3">
                                    <label for="">Status</label>
                                    <input type="checkbox" width="70px" height="70px"  <?=$row['statustown'] == '1' ? 'checked':'' ?>  />
                                </div>
                    
                                <div class="col-md-12 mb-3">
                                    <button type="submit" class="btn btn-primary" name="townupdate">update land</button>
                                </div>
                            </form>
                    <?php
