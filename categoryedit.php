...
//                enable the town select dropdown
        ...
        <body class="sb-nav-fixed">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4>EDIT LAND
                                    <a href="categoryview.php" class="btn btn-danger float-end">back</a>
                                </h4>
                            </div>
                            <div class="card-body">
                                ...
                                if (isset($_GET['id'])) {
                                    $land_id = $_GET['id'];
                                    $land_query = "SELECT * FROM land WHERE idland='$land_id' LIMIT 1";
                                    $land_run = mysqli_query($con, $land_query);

                                    if (mysqli_num_rows($land_run) > 0) {
                                        $row = mysqli_fetch_array($land_run);
                                        ?> 

                                        <form action="code.php" method="POST" enctype="multipart/form-data">
                                            <div class="row">
                                                <label for="">ID</label>
                                                <input type="hidden" required name="id"  value="<?= $row['idland']; ?>" required="" class="form-control">
                                                    ...
                                                    if (mysqli_num_rows($regionl_run) > 0) {
                                                        ...
                                                        <select name="idregion" required="" class="form-control">
                                                            ...
                                                            foreach ($regionl_run as $regionl_item) {
                                                                ?>
                                                                <option value="<?= $regionl_item['idregion'] ?>" <?= $regionl_item['idregion'] == $row['idregion'] ? 'selected' : '' ?> ><?= $regionl_item['nameregion'] ?></option>
                                                                <?php
                                                            }
                                                            ?>
                                                        </select>
                                                    ...
                                                    if (mysqli_num_rows($townl_run) > 0) {
                                                        ...
                                                        <select name="idtown" required="" class="form-control">
                                                            ...
                                                            foreach ($townl_run as $townl_item) {
                                                                ?>
                                                                <option value="<?= $townl_item['idtown'] ?>" <?= $townl_item['idtown'] == $row['idtown'] ? 'selected' : '' ?> ><?= $townl_item['nametown'] ?></option>
                                                                <?php
                                                            }
                                                            ?>
                                                        </select>
                                                ...
                                                <div class="col-md-6 mb-3">
                                                    ...
                                                    <?php
                                                    $districtl = "SELECT * FROM district WHERE statusdistrict='0' ";
                                                    $districtl_run = mysqli_query($con, $districtl);
                                                    if (mysqli_num_rows($districtl_run) > 0) {
                                                        ?>
                                                        <select name="iddistrict" required="" class="form-control">
                                                            <option value="">------ Click To Select District----</option>
                                                            <?php
                                                            foreach ($districtl_run as $districtl_item) {
                                                                ?>
                                                                <option value="<?= $districtl_item['iddistrict'] ?>" <?= $districtl_item['iddistrict'] == $row['iddistrict'] ? 'selected' : '' ?> ><?= $districtl_item['namedistrict'] ?></option>
                                                                <?php
                                                            }
                                                            ?>
                                                        </select>
                                                ...
                                                <div class="col-md-6 mb-3">
                                                    ...
                                                    <input type="text"  name="titleland" required="" value="<?= $row['titleland'] ?>" class="form-control">
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Land description</label>
                                                    <textarea name="description"  required="" id="summernote" class="form-control "  rows="4"><?= $row['descriptionland'] ?></textarea>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Unite Price</label>
                                                    <input type="text" required name="uniteprice" value="<?= $row['uniteprice'] ?>" required="" class="form-control">
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Total Price</label>
                                                    <input type="text" required name="totalprice" value="<?= $row['totalprice'] ?>" required="" class="form-control">
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Land Location</label>
                                                    <input type="text" required name="locationland" value="<?= $row['locationland'] ?>" required="" class="form-control">
                                                </div>
                                                
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Size</label>
                                                    <input type="text" required name="size" value="<?= $row['size'] ?>" required="" class="form-control">
                                                </div>
                                                
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Land Purpose</label>
                                                    <input type="text" required name="landpurpose" value="<?= $row['land_purpose'] ?>" required="" class="form-control">
                                                </div>
                                                
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Land Type</label>
                                                    <input type="text" required name="landtype" value="<?= $row['land_type'] ?>" required="" class="form-control">
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label for="">Land Image</label>
                                                    <input type="hidden" required name="old_image" value="<?= $row['imageland'] ?>" class="form-control" />
                                                ...
                                                <div class="col-md-6 mb-3">
                                                    ...
                                                    <input type="hidden" name="statusland" value="available">
                                                    <input type="checkbox" name="statusland" value="reserved" <?= $row['statusland'] == 'reserved' ? 'checked' : '' ?> />
                                                    <label for="">Check if reserved</label>
                                                </div>

                                                <div class="col-md-12 mb-3">
                                                    <button type="submit" class="btn btn-primary" name="categoryupdate">Update Town</button>
                                                </div>
                                            </div>
                                        </form> 
                                        <?php
            ...
            <script>
             ...
             <script>
      $('#summernote').summernote({
        placeholder: 'type and design your description',
        height: 100
      });
    </script>
            <!-- //Summernote JS - CDN Link -->
