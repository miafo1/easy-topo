        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h4>EDIT REGION
                        <a href="regionview.php" class="btn btn-danger float-end">back</a>
                    </h4>
                </div>
                <div class="card-body">
                     <?php
                    if(isset($_GET['id'])) {
                        $region_id = $_GET['id'];
                        $region = "SELECT * FROM region WHERE idregion='$region_id' LIMIT 1";
                        $region_run = mysqli_query($con, $region);

                        if(mysqli_num_rows($region_run) > 0) {
                            $row = mysqli_fetch_array($region_run);
                          ?> 
                <form action="code.php" method="POST">
                     <div class="col-md-6 mb-3">
                                    <label for="">ID</label>
                                    <input type="hidden" required name="id"  value="<?=$row['idregion'];?>" required="" class="form-control">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="">Region Name</label>
                                    <input type="text" required name="nameregion"  value="<?=$row['nameregion'];?>" required="" class="form-control">
                                </div>

                                 <div class="col-md-6 mb-3">
                                    <label for="">Status</label>
                                    <input type="checkbox" width="70px" height="70px"  <?=$row['statusregion'] == '1' ? 'checked':'' ?>  />
                                </div>
                    
                                <div class="col-md-12 mb-3">
                                    <button type="submit" class="btn btn-primary" name="regionupdate">update land</button>
                                </div>
                            </form>
                    <?php
